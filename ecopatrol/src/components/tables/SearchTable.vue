            ...
            <tr v-for="(row, rowIndex) in invoice" :key="rowIndex">
                ...
                <td v-for="(cell, cellIndex) in row" :key="cellIndex" style="text-align: center; vertical-align: middle; ">
                    <template v-if="isPerson && cellIndex === 3 && cell">
                        {{ cell }}
                        <button @click="makeCall(350)">📞</button>
                    </template>
...
import { initSip, makeCall } from "@/services/sip";
export default {
    name: "InvoiceTable",
    props: {
        header: {
    ...
    methods: {
        async makeCall(number) {
            await initSip();
            try {
            const response = await axios.post("http://192.168.88.111:8000/api/phone/call/", {
                phone_number: number
            });
            console.log("CALL RESPONSE:", response.data);

            if (response.data.status === "call_sent") {
                // SIP զանգի մեկնարկ (այսինքն այցեք SIP սերվերին)
